          ...
          >
            <div class="p-3">
              ...
              <div class="mt-2 text-center">
                ...
                <p class="text-xs text-gray-500 truncate">{{ template.category }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="mt-6 bg-white shadow overflow-hidden sm:rounded-md">
          <div v-if="filteredDashboards.length === 0" class="text-center py-12">
            ...
            <p class="mt-1 text-sm text-gray-500">
              {{ searchQuery || selectedCategoryFilter ? 'Try adjusting your search terms or filters.' : 'Get started by creating your first dashboard.' }}
            </p>
          </div>
              ...
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span
                    ...
                    :class="getCategoryBadgeClass(dashboard.category || 'General')"
                  >
...
const availableCategories = computed(() => {
  return dashboardStore.getUniqueCategories
})
...
const filteredDashboards = computed(() => {
  ...
  if (selectedCategoryFilter.value) {
    filtered = filtered.filter(dashboard => 
      (dashboard.category || 'General') === selectedCategoryFilter.value
    )
  }
...
const createFromTemplate = (template: any) => {
  // Create a dashboard with the template's category
