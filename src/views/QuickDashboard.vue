                ...
                <div class="mt-2 border-t pt-2">
                  <div class="flex items-center justify-between mb-1">
                    <button
                      ...
                      class="text-xs font-semibold text-primary-700 hover:text-primary-800 hover:underline"
                    >
                      Custom Fields
                    </button>
          ...
          <div class="fixed inset-0 overflow-y-auto">
            <div class="flex min-h-full items-center justify-center p-4 text-center">
              <TransitionChild
                ...
                leave-to="opacity-0 scale-95"
              >
                <DialogPanel class="relative transform overflow-hidden rounded-2xl bg-white shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-4xl">
                      ...
                      <div class="flex items-center">
                        ...
                        <div class="ml-4">
                          ...
                          <p class="text-sm text-primary-100 mt-1">
                            {{ selectedDataFieldsSource?.name || 'Data Source' }}
                          </p>
                        </div>
                      </div>
                      <button
                  ...
                  <div class="border-b border-gray-200">
                    <nav class="-mb-px flex space-x-8 px-6" aria-label="Tabs">
                      <button
                        @click="activeDataFieldsTab = 'overview'"
                        :class="[
                          'whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200',
                          activeDataFieldsTab === 'overview'
                      ...
                      <button
                        @click="activeDataFieldsTab = 'custom'"
                        :class="[
                          'whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200',
                          activeDataFieldsTab === 'custom'
                            ? 'border-primary-500 text-primary-600'
                            : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                        ]"
                      >
                  ...
                  <div class="px-6 py-6">
                    <!-- Overview Tab -->
                    <div v-if="activeDataFieldsTab === 'overview'" class="space-y-6">
                      <!-- Data Source Info -->
                      <div class="bg-gray-50 rounded-lg p-4">
                        <div class="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            <span class="font-medium text-gray-700">Total Rows:</span>
                            <span class="ml-2 text-gray-900">{{ selectedDataFieldsSource?.rows.length || 0 }}</span>
                          </div>
                          <div>
                            <span class="font-medium text-gray-700">Total Fields:</span>
                            <span class="ml-2 text-gray-900">{{ selectedDataFieldsSource?.columns.length || 0 }}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-300">
                          ...
                          <tbody class="bg-white divide-y divide-gray-200">
                            <tr
                              v-for="(column, index) in selectedDataFieldsSource?.columns.filter(c => !c.isCustom)"
                              :key="column.name"
                            ...
                            >
                              ...
                              <td class="px-6 py-4 text-sm text-gray-500">
                                <div class="flex flex-wrap gap-1">
                                  <span
                                    v-for="(value, idx) in column.values.filter(v => v != null && v !== '').slice(0, 3)"
                                    :key="idx"
                    ...
                    <!-- Custom Fields Tab -->
                    <div v-if="activeDataFieldsTab === 'custom'" class="space-y-6">
                      <!-- Add Button -->
                      <div class="flex justify-end">
                        <button
                          @click="openCustomFieldModal(selectedDataFieldsSource)"
                      ...
                      <!-- Custom Fields Table -->
                      <div v-if="selectedDataFieldsSource?.columns.filter(c => c.isCustom).length > 0" class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-300">
                          <thead class="bg-gray-50">
                            <tr>
                              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                          ...
                          <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            >
                              ...
                              <td class="px-6 py-4 text-sm text-gray-900">
                                <code class="bg-gray-100 px-2 py-1 rounded text-xs">{{ column.expression }}</code>
                              </td>
                              <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                  class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                              ...
                              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-2">
                                  <button
                                    @click="editCustomFieldModal(selectedDataFieldsSource, column)"
                                  ...
                                  <button
                                    @click="removeCustomFieldModal(selectedDataFieldsSource, column)"
...
const selectedDataFieldsSource = ref<any>(null)
const activeDataFieldsTab = ref<'overview' | 'custom'>('overview')

function openDataFieldsDialog(ds: any) {
  selectedDataFieldsSource.value = ds
  showDataFieldsDialog.value = true
  activeDataFieldsTab.value = 'overview'
}

function closeDataFieldsDialog() {
  showDataFieldsDialog.value = false
  selectedDataFieldsSource.value = null
}
