    ...
    <div class="flex h-[calc(100vh-4rem)]">
          ...
          <div v-else class="space-y-3">
            ...
            <div v-if="selectedChartType === 'card'">
              ...
              <div class="mt-2">
                ...
                >
                  {{ chartConfig.previousMetric || 'Drop previous period metric here (numbers only)' }}
                </div>
              </div>
      ...
      <div :class="['flex-1 p-6', previewMode ? 'bg-gray-900' : '']" style="position:relative;">
        <div class="bg-white rounded-lg shadow-sm h-full">
          <!-- Tab Navigation -->
          <div v-if="Object.keys(dashboardTabs).length > 1 || !hideTabBar" class="border-b border-gray-200 px-6 pt-4">
            <div class="flex items-center justify-between">
              <div class="flex space-x-1">
                <button
                  v-for="(tab, tabId) in dashboardTabs"
                  :key="tabId"
              ...
              <div class="flex items-center space-x-2">
                ...
                >
                  {{ hideTabBar ? 'Show Tabs' : 'Hide Tabs' }}
                </button>
                <button
                  @click="addTab"
                  class="inline-flex items-center px-2 py-1 text-xs font-medium rounded text-primary-600 hover:bg-primary-50"
                >
...
const currentTab = computed(() => {
  return dashboardTabs.value[activeTabId.value] || { charts: [] }
})

const addTab = () => {
  const tabCount = Object.keys(dashboardTabs.value).length
...
const removeTab = (tabId: string) => {
  ...
  if (confirm('Are you sure you want to remove this tab and all its charts?')) {
...
const removeChart = (chartId: string) => {
  if (confirm('Are you sure you want to remove this chart?')) {
    currentTab.value.charts = currentTab.value.charts.filter(chart => chart.id !== chartId)
...
onMounted(async () => {
  ...
  if (dashboardId) {
    ...
    if (dashboard) {
      ...
      // Load charts for this dashboard into first tab
      ...
      dashboardTabs.value[activeTabId.value].charts = charts
      await nextTick()
      initializeGridStack()
    }
  }
