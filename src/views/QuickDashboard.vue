          ...
          <div class="flex items-center justify-between mb-2">
            <div class="flex items-center space-x-1 overflow-x-auto">
              <button
                ...
                :class="[
                  'flex items-center px-4 py-2 text-sm font-medium rounded-t-lg border-b-2 transition-all duration-200 group relative',
                  activeTabId === tab.id
              ...
              >
                <input
                  ...
                  @keyup.escape="cancelTabEdit"
                  class="bg-transparent border-none focus:ring-0 text-sm font-medium w-24"
                  @click.stop
                />
              ...
              <button
                @click="addNewTab"
                class="flex items-center px-3 py-2 text-sm font-medium text-gray-500 hover:text-gray-700 border border-dashed border-gray-300 hover:border-gray-400 rounded-lg transition-colors duration-200"
              >
  ...
  if (confirm('Are you sure you want to remove this tab? All charts in this tab will be deleted.')) {
    ...
    if (tabIndex !== -1) {
      ...
      if (activeTabId.value === tabId) {
        activeTabId.value = dashboardTabs.value[Math.max(0, tabIndex - 1)].id
      }
...
const getCurrentTabCharts = () => {
  const currentTab = dashboardTabs.value.find(tab => tab.id === activeTabId.value)
  return currentTab ? currentTab.charts : []
}
...
const addOrUpdateChart = () => {
  ...
  if (editingChartId.value) {
    // Update existing chart in current tab
    const currentTab = dashboardTabs.value.find(tab => tab.id === activeTabId.value)
    if (currentTab) {
      const idx = currentTab.charts.findIndex(c => c.id === editingChartId.value)
      if (idx !== -1) {
        currentTab.charts[idx].config = {
          ...currentTab.charts[idx].config,
          id: currentTab.charts[idx].id,
          ...
          createdAt: currentTab.charts[idx].config.createdAt || new Date()
        }
      }
    }
...
const addChart = () => {
  ...
  const currentTab = dashboardTabs.value.find(tab => tab.id === activeTabId.value)
  if (currentTab) {
    currentTab.charts.push(newChart)
  }
...
const removeChart = (chartId: string) => {
  if (confirm('Are you sure you want to remove this chart?')) {
    const currentTab = dashboardTabs.value.find(tab => tab.id === activeTabId.value)
    if (currentTab) {
      currentTab.charts = currentTab.charts.filter(chart => chart.id !== chartId)
    }
...
const initializeGridStack = async () => {
  ...
  try {
    ...
    gridStack.on('change', (event, items) => {
      ...
      if (currentTab) {
        items.forEach(item => {
          const chart = currentTab.charts.find(c => c.id === item.id)
...
onMounted(async () => {
  ...
  if (dashboardId) {
    ...
    if (dashboard) {
      ...
      if (charts.length > 0) {
        dashboardTabs.value[0].charts = charts
      }
