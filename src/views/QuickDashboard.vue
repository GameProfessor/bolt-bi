        ...
        <div class="bg-white border-b border-gray-200 px-6 pt-4">
          <div class="flex items-center justify-between mb-2">
            <div class="flex items-center space-x-1 overflow-x-auto">
              <button
                v-for="tab in Object.values(dashboardTabs)"
                :key="tab.id"
                @click="activeTabId = tab.id"
                :class="[
                  'flex items-center px-4 py-2 text-sm font-medium rounded-t-lg border-b-2 transition-all duration-200 whitespace-nowrap',
                  activeTabId === tab.id
              ...
              >
                <span>{{ tab.name }}</span>
                <button
                  v-if="Object.keys(dashboardTabs).length > 1"
              ...
              <button
                @click="addNewTab"
                class="flex items-center px-3 py-2 text-sm font-medium text-gray-500 hover:text-gray-700 border border-dashed border-gray-300 rounded-lg hover:border-gray-400 transition-all duration-200"
              >
            ...
            <div class="flex items-center space-x-2">
              <input
                ...
                class="text-sm border border-gray-300 rounded px-2 py-1 w-32 focus:ring-1 focus:ring-primary-500 focus:border-primary-500"
              />
            </div>
          </div>
        </div>
...
const gridStackInstances = ref<Record<string, GridStack>>({})
...
const removeTab = (tabId: string) => {
  if (Object.keys(dashboardTabs.value).length <= 1) {
    showToastNotification('warning', 'Cannot Remove Tab', 'At least one tab must remain.')
    return
  }
  
  if (confirm('Are you sure you want to remove this tab? All charts in this tab will be lost.')) {
    // Destroy GridStack instance for this tab
    if (gridStackInstances.value[tabId]) {
      gridStackInstances.value[tabId].destroy(false)
      delete gridStackInstances.value[tabId]
    }
    
    delete dashboardTabs.value[tabId]
    
    // Switch to another tab if the current one was removed
...
const setGridStackRef = (el: HTMLElement | null, tabId: string) => {
  if (el && tabId === activeTabId.value) {
    nextTick(() => {
      initializeGridStack(tabId, el)
    })
  }
}

const initializeGridStack = async (tabId: string, container: HTMLElement) => {
  if (!container || !dashboardTabs.value[tabId] || dashboardTabs.value[tabId].charts.length === 0) return
  ...
  try {
    ...
    gridStackInstances.value[tabId] = gridStack

    // Listen for layout changes
    gridStack.on('change', (event, items) => {
      items.forEach(item => {
        const chart = dashboardTabs.value[tabId].charts.find(c => c.id === item.id)
...
const addOrUpdateChart = () => {
  ...
  if (editingChartId.value) {
    ...
    const idx = currentTab.value.charts.findIndex(c => c.id === editingChartId.value)
    if (idx !== -1) {
      currentTab.value.charts[idx].config = {
        ...currentTab.value.charts[idx].config,
        id: currentTab.value.charts[idx].id,
        ...
        createdAt: currentTab.value.charts[idx].config.createdAt || new Date()
      }
    }
    editingChartId.value = null
    resetChartConfig()
    nextTick(() => {
      const container = document.querySelector(`[data-tab-id="${activeTabId.value}"] .grid-stack`) as HTMLElement
      if (container) initializeGridStack(activeTabId.value, container)
    })
    return
  }
  // Add new chart
...
const saveDashboard = () => {
  const allCharts = Object.values(dashboardTabs.value).flatMap(tab => tab.charts)
  if (!dashboardName.value || allCharts.length === 0) return
...
onMounted(async () => {
  ...
  if (dashboardId) {
    ...
    if (dashboard) {
      ...
      const firstTabId = Object.keys(dashboardTabs.value)[0]
      if (firstTabId) {
        dashboardTabs.value[firstTabId].charts = dashboard.widgets.map(widget => {
...
onUnmounted(() => {
  ...
  Object.values(gridStackInstances.value).forEach(gridStack => {
    gridStack.destroy(false)
  })
  gridStackInstances.value = {}
